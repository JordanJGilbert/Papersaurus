...
import { ChevronLeft, ChevronRight, Play, Pause, RotateCcw, Maximize2, X, Edit3, Loader2, Sparkles } from "lucide-react";
...
export default function CardPreview({ card, onCardUpdate }: CardPreviewProps) {
  ...
  const [editedCard, setEditedCard] = useState<GeneratedCard>(card);
  
  // Use the custom hook to get split images
  const [splitImages, isLoadingSplits] = useSplitImages(editedCard);

  // Update edited card when original card changes
  useEffect(() => {
    setEditedCard(card);
  }, [card]);

  // Edit functionality
  const handleEditSection = (sectionId: string, title: string, description: string, imageUrl: string, originalImageUrl: string) => {
    setEditingSection({
      id: sectionId,
      title,
      description,
    ...
    try {
      const response = await fetch(`${BACKEND_API_BASE_URL}/internal/call_mcp_tool`, {
        ...
        body: JSON.stringify({
          ...
          arguments: {
            images: [editingSection.originalImageUrl],
      ...
      if (result.status === 'success' && result.results && result.results.length > 0) {
        ...
        if (editResult.status === 'success' && editResult.edited_url) {
          // Update the appropriate image in the card
          const newCard = { ...editedCard };
          
          if (editingSection.id === 'front-cover' || editingSection.id === 'back-cover') {
            newCard.frontCover = editResult.edited_url;
          } else if (editingSection.id === 'interior') {
            newCard.leftPage = editResult.edited_url;
          }
      ...
      toast.error(`Failed to edit ${editingSection.title.toLowerCase()}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsEditing(false);
    }
  };
  ...
  const slides = [
    ...
    },
    {
      ...
      image: card.leftPage, // Show the raw interior image without splitting
      ...
      type: "single" as const, // Changed from "double" to "single"
    },
    {
      id: "back-cover",
      title: "Back Cover",
      subtitle: "What's on the back",
      image: splitImages.backCover,
          ...
          <AnimatePresence initial={false} custom={direction}>
            ...
            >
              <div className="h-full flex flex-col">
                ...
                {/* Image Container - Shows single images */}
                <motion.div 
                  className="flex-1 relative border-2 border-white/20 rounded-lg overflow-hidden shadow-2xl bg-white"
